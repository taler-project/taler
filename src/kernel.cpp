...
static bool GetKernelStakeModifier(const CBlock& blockFrom, unsigned int nCoinStakeTime, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    {
        //uint64_t tmp = (int64_t)nCoinStakeTime-((int64_t)nStakeModifierTime+(int64_t)(Params().GetConsensus().nStakeMinAge)-(int64_t)nStakeModifierSelectionInterval);
        //LogPrintf("nHeight:%.9d:SecondWaitPOS = %.5d - %.2d:%.2d:%.2d\n",mapBlockIndex[blockFrom.GetHash()]->nHeight,tmp/60/60/24,tmp/60/60%24,tmp/60%60,tmp%60);
...
bool CheckStakeKernelHash(unsigned int nBits, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTxOut& txOutPrev, const COutPoint& prevout, uint32_t nTimeTx, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (!GetKernelStakeModifier(blockFrom,blockFrom.GetBlockTime(), nStakeModifier, nStakeModifierHeight, nStakeModifierTime, fPrintProofOfStake))
        return false;
    ...
    {
        //if(bnCoinDayWeight * bnTargetPerCoinDay>1000*bnTargetPerCoinDay)
        //    LogPrintf("nHeight:%.9d:(%s)>(%s)\n",mapBlockIndex[blockFrom.GetHash()]->nHeight,UintToArith256(hashProofOfStake).ToString(),(bnCoinDayWeight*bnTargetPerCoinDay).ToString());
        return false;
    }
    if (!fPrintProofOfStake) {
