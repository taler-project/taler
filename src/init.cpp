 ...
 * between talerd, and taler-qt and non-server tools.
*/
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://talercrypto.com>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 2017, COPYRIGHT_YEAR) + " ") + "\n"/* + CopyrightAtomHolders(strprintf(_("Copyright (C) 2019 "))) + "\n"*/
           "\n" +
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if taler is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if taler is started while in a temporary directory.\n",
