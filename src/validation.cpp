...
bool static IsTLRHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.TLRHeight;
}

bool IsTLRHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsTLRHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsTLRHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsTLRHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    // Check the header
    if (block.IsProofOfWork() && !CheckProofOfWork(block.GetPoWHash(nHeight, consensusParams), nHeight, block.nBits, consensusParams))
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip()->nChainWork < nMinimumChainWork)
        return true;
    if (fSkipHardforkIBD && chainActive.Tip()->nHeight + 1 >= Params().GetConsensus().TLRHeight)
        return false;
    int64_t target_time = 0 ? (int64_t)Params().GetConsensus().BitcoinPostforkTime : GetTime();
...
bool PoSContextualBlockChecks(const CBlock& block, CValidationState& state, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (block.IsProofOfStake()) {
        if(pindex->nHeight <= Params().GetConsensus().TLRHeight + Params().GetConsensus().TLRInitLim) {
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (block.IsProofOfStake() && i == 1) {
                ...
                posReward = GetProofOfStakeReward(nCoinAge,pindex->nPowHeight,chainparams.GetConsensus());
            }
...
static bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    if (fCheckPOW && !block.IsProofOfStake() && !CheckProofOfWork(block.GetPoWHash(nHeight, consensusParams), nHeight, block.nBits, consensusParams))
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock)
{
    ...
    if (block.IsNewFormatBlock() != IsTLRHardForkEnabled(nHeight, chainparams.GetConsensus())) {
