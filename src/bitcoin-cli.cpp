...
std::string HelpMessageCli()
{
    ...
    strUsage += HelpMessageOpt("-rpcwallet=<walletname>", _("Send RPC for non-default wallet on RPC server (argument is wallet filename in talerd directory, required if talerd/-Qt runs with multiple wallets)"));
    strUsage += HelpMessageOpt("-convertaddress=<bitcoin_address>", _("Convert a Bitcoin address into Taler address format."));
                ...
                if (!error.isNull()) {
                    ...
                    if (error.isObject())
                    {
                        ...
                        if (errCode.isNum() && errCode.get_int() == RPC_WALLET_NOT_SPECIFIED) {
                            strPrint += "\nTry adding \"-rpcwallet=<filename>\" option to taler-cli command line.";
                        }
                    }
                } else {
                    // Result
