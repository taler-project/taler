...
###   bitcoincore.org and talercrypto.com and compares them.
...
if [ $? -ne 0 ]; then
   echo "talercrypto.com failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "talercrypto.com and bitcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Taler Core binary release signing key installed?"
   fi
